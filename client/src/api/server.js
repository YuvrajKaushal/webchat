...
const Service = {
  ...
  // 获取当前房间所有历史记录
  getRoomHistory: (data) =>
    Axios.get('/api/message/history/byUser', {
      params: data
    }),
  RoomHistoryAll: (data) =>
    Axios.get('/api/message/v2/history', {
      params: data
    }),
  // 查询最火用户
  getHostUserList: (data) =>
    Axios.get('/api/message/getHot', {
      params: data
    }),
  // 查询 vip 用户
  getvipuser: (data) =>
    Axios.get('/api/user/vipuser', {
      params: data
    }),
  getSearch: (data) =>
  ...
  // 机器人
  getRobotMessage: (data) =>
    Axios.get('/api/message/robotapi', {
      params: data
    }),
  // 上传图片
  postUploadFile: (data) =>
  ...
  postUploadAvatar: (data) =>
    Axios.post('/api/file/avatar', data, {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
    }),
  // 查询用户信息
  getUserInfo: (data) =>
